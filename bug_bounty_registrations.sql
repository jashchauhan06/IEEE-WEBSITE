...
-- This table is specifically designed for the bug bounty hackathon event

CREATE TABLE IF NOT EXISTS bug_bounty_registrations (
    ...
    member3_semester INTEGER NOT NULL CHECK (member3_semester >= 3 AND member3_semester <= 8),
    member3_section VARCHAR(10) NOT NULL,
    member3_prn VARCHAR(50) NOT NULL,
    ...
    UNIQUE(member3_email), -- Prevent duplicate member registrations
    ...
    CONSTRAINT valid_member3_email CHECK (member3_email ~* '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}$'),
    ...
    CONSTRAINT valid_member3_phone CHECK (member3_phone ~* '^[+]?[0-9\s\-\(\)]{10,15}$')
);

-- Create indexes for better performance
CREATE INDEX IF NOT EXISTS idx_bug_bounty_registrations_team_name ON bug_bounty_registrations(team_name);
CREATE INDEX IF NOT EXISTS idx_bug_bounty_registrations_leader_email ON bug_bounty_registrations(leader_email);
CREATE INDEX IF NOT EXISTS idx_bug_bounty_registrations_status ON bug_bounty_registrations(registration_status);
CREATE INDEX IF NOT EXISTS idx_bug_bounty_registrations_created_at ON bug_bounty_registrations(created_at);
CREATE INDEX IF NOT EXISTS idx_bug_bounty_registrations_leader_semester ON bug_bounty_registrations(leader_semester);
...
COMMENT ON TABLE bug_bounty_registrations IS 'Registrations specifically for BUG BOUNTY HACKATHON event - Second and Third Year Students Only';
...
COMMENT ON COLUMN bug_bounty_registrations.member3_semester IS 'Must be between 3-8 (2nd and 3rd year students) as per event requirements';
...
--     team_name, leader_name, leader_phone, leader_email, leader_semester, leader_section, leader_prn,
--     member2_name, member2_phone, member2_email, member2_semester, member2_section, member2_prn,
--     member3_name, member3_phone, member3_email, member3_semester, member3_section, member3_prn,
--     preferred_programming_languages, team_motivation, registration_status
-- ) VALUES (
--     'Bug Hunters', 'John Doe', '+91-9876543210', 'john.doe@example.com', 4, 'A', 'SIT2023001',
--     'Jane Smith', '+91-9876543211', 'jane.smith@example.com', 4, 'A', 'SIT2023002',
--     'Bob Johnson', '+91-9876543212', 'bob.johnson@example.com', 4, 'A', 'SIT2023003',
--     ARRAY['Python', 'JavaScript', 'C++'], 'We want to improve our cybersecurity skills', 'Pending'
-- );
